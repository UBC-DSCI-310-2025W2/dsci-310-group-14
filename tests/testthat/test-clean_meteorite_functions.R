...
test_that("clean_meteorite_data errors if input is not a data frame", {
  expect_error(
    clean_meteorite_data("not_a_dataframe"),
    "`df` must be a data frame."
  )
})

test_that("clean_meteorite_data errors if required columns are missing", {
  bad_df <- tibble(
    year = c("2001", "2002"),
    reclat = c("1", "2"),
    reclong = c("5", "6")
  ...
  expect_error(
    clean_meteorite_data(bad_df),
    "Missing required columns:"
  )
})

test_that("clean_meteorite_data errors if numeric conversion fails", {
  bad_df <- tibble(
    mass..g. = c("10", "abc"),
    year = c("2001", "2002"),
    reclat = c("1", "2"),
